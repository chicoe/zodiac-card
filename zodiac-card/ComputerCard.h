...
void __not_in_flash_func(ComputerCard::BufferFull)()
{
    ...
    // Update GPIOs for next mux state
    gpio_put(MX_A, next_mux_state & 1);
	gpio_put(MX_B, (next_mux_state >> 1) & 1);
	...
	adcInR = -((((int32_t)ADC_Buffer[cpuPhase][0] + (int32_t)ADC_Buffer[cpuPhase][4]) - 0x1000) >> 1);

	adcInL = -((((int32_t)ADC_Buffer[cpuPhase][1] + (int32_t)ADC_Buffer[cpuPhase][5]) - 0x1000) >> 1);
